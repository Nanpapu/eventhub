...
// Danh sách categories chuẩn cho toàn ứng dụng
...
// Lấy tên của category từ ID
export const getCategoryName = (categoryId: string): string => {
  const category = categories.find((cat) => cat.id === categoryId);
  return category ? category.name : categoryId;
};

// Lấy danh sách các categories theo định dạng cho dropdown
...
// Lấy hình ảnh của category theo ID
